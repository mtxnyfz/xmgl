    ...
    function initWebUpload(item, options) {
        ...
        if (flag == 1) {
            if (typeof guidGenerator != 'undefined')
                ...
                pickerid = S4();
           // uploaderStrdiv = '<div class="webuploader">' +
           ...
           //    '</div>' +
            ...
            uploaderStrdiv = '<div class="webuploader">' +
               ...
               '<div id="' + pickerid + '" style="color:#FF0000">选择文件</div>' +
        ...
        var jsonData = {
         ...
         webuploaderoptions = $.extend({
            ...
            // 文件接收服务端。
            server: 'upload.ashx?lx=' + up_type + '&extensions=' + extensions + '&up_type_zm=' + up_type_zm,
        ...
        //alert(webuploaderoptions.server);
        ...
        uploader.on('uploadProgress', function (file, percentage) {
            ...
            $li.find("span.text").text('当前进度：' + Math.round(percentage * 100) + '%' + '，状态：未上传完毕');
            $percent.css('width', percentage * 100 + '%');
        });
        uploader.on('uploadSuccess', function (file, response) {
            target.find('#' + file.id).find('div.state').text('已上传完毕');
            ...
            if( hiddenid =="Hidden1")
                jsonDatahid1.fileList.push(fileEvent)
            else if(hiddenid == "Hidden2")
                jsonDatahid2.fileList.push(fileEvent)
            else if (hiddenid == "Hidden3")
                jsonDatahid3.fileList.push(fileEvent)
            else if (hiddenid == "Hidden4")
                jsonDatahid4.fileList.push(fileEvent)
            else if (hiddenid == "Hidden5")
                jsonDatahid5.fileList.push(fileEvent)
            else if (hiddenid == "Hidden6")
                jsonDatahid6.fileList.push(fileEvent)
            else if (hiddenid == "Hidden7")
                jsonDatahid7.fileList.push(fileEvent)
            else if (hiddenid == "Hidden8")
                jsonDatahid8.fileList.push(fileEvent)
        ...
        uploader.on('uploadComplete', function (file) {
            ...
            if (hiddenid == "Hidden1")
                $("#" + hiddenid).val(JSON.stringify(jsonDatahid1));
            else if (hiddenid == "Hidden2")
                $("#" + hiddenid).val(JSON.stringify(jsonDatahid2));
            else if (hiddenid == "Hidden3")
                $("#" + hiddenid).val(JSON.stringify(jsonDatahid3));
            else if (hiddenid == "Hidden4")
                $("#" + hiddenid).val(JSON.stringify(jsonDatahid4));
            else if (hiddenid == "Hidden5")
                $("#" + hiddenid).val(JSON.stringify(jsonDatahid5));
            else if (hiddenid == "Hidden6")
                $("#" + hiddenid).val(JSON.stringify(jsonDatahid6));
            else if (hiddenid == "Hidden7")
                $("#" + hiddenid).val(JSON.stringify(jsonDatahid7));
            else if (hiddenid == "Hidden8")
                $("#" + hiddenid).val(JSON.stringify(jsonDatahid8));
    ...
    $.fn.powerWebUpload = function (options) {
        ...
        $(ele).each(function (idnex, item) {
            ...
            if (typeof PowerJs != 'undefined') {
                ...
                PowerJs.Core.lazyLoad(applicationPath + "/Scripts/lib/webuploader/webuploader.js", function () {
