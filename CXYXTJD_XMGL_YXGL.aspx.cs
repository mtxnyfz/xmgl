...
{
    public partial class CXYXTJD_XMGL_YXGL : System.Web.UI.Page
    {
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "tj")
            {
                ...
                {
                    string sqlstr = "update CJYXTJD_XM set ZT=3 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        databind();
                        Alert.Show("操作成功");
            ...
            else if (e.CommandName == "th")
            {
                if (Convert.ToInt32(zt) == 2)
                {
                    string sqlstr = "update CJYXTJD_XM set ZT=4 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        databind();
                        Alert.Show("操作成功");
