...
{
    public partial class CXYXTJD_XMGL : System.Web.UI.Page
    {
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "up")
            {
                if (zt == "1" || zt == "4" || zt == "6")
                {
                    PageContext.RegisterStartupScript(Window1.GetShowReference("CXYXTJD_XMGL_Edit.aspx?id=" + selectedID, "修改申报书") + Window1.GetMaximizeReference());
                }
                else
                {
            ...
            else if (e.CommandName == "tj")
            {
                if (zt == "1" || zt == "4" || zt == "6")
                {
                    string sqlstr = "update CJYXTJD_XM set ZT=2 where ID=" + Convert.ToInt32(selectedID);
            ...
            else if (e.CommandName == "del")
            {
                if (zt == "1")
                {
                    string sqlstr = "update CJYXTJD_XM set SFSC=1 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        sqlstr = "delete CJYXTJD_JDYTZYQK  where XMBH='" + xmbh + "';delete CJYXTJD_XMYSZB  where XMBH='" + xmbh + "'";
