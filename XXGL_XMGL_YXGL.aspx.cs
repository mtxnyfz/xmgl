...
{
    public partial class XXGL_XMGL_YXGL : System.Web.UI.Page
    {
        ...
        protected void databind()
        {
            ...
            //sqlstr = "SELECT dbo.TYL_XM.ID, dbo.TYL_XM.XMBH, dbo.TYL_XM.XMMC, dbo.TYL_XM.JFYS_ZXJF, dbo.TYL_XM.JFYS_XXPTJF, dbo.XMFJ.XMYSMXWJM, dbo.TYL_XM.SFSC, dbo.TYL_XM.ZT, dbo.TYL_XM.User_Uid FROM dbo.TYL_XM LEFT JOIN dbo.XMFJ ON dbo.TYL_XM.XMBH = dbo.XMFJ.XMBH WHERE  SFSC != 1 and ZT>=2 and DWMC='" + ViewState["xxmc"].ToString() + "'"; 
            sqlstr = "SELECT dbo.XXGLPT_XM.ID, dbo.XXGLPT_XM.XMBH, dbo.XXGLPT_XM.XMMC, dbo.XXGLPT_XM.JFYS_ZXJF, dbo.XXGLPT_XM.JFYS_XXPTJF, dbo.XMFJ.XMYSMXWJM, dbo.XXGLPT_XM.SFSC, dbo.XXGLPT_XM.ZT, dbo.XXGLPT_XM.User_Uid FROM dbo.XXGLPT_XM INNER JOIN dbo.XMFJ ON dbo.XXGLPT_XM.XMBH = dbo.XMFJ.XMBH WHERE  SFSC != 1 and ZT>=2 and DWMC='" + ViewState["xxmc"].ToString() + "'"; 
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (selectedCount > 0 && selectedCount < 2)
            {
                ...
                if (zt == "1")
                {
                    sqlstr = "update XXGLPT_XM set SFSC=1 where ID=" + Convert.ToInt32(selectedID);
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "tj")
            {
                ...
                {
                    string sqlstr = "update XXGLPT_XM set ZT=3 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        databind();
                        Alert.Show("操作成功");
            ...
            else if (e.CommandName == "th")
            {
                ...
                {
                    string sqlstr = "update XXGLPT_XM set ZT=4 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        databind();
                        Alert.Show("操作成功");
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            if (selectedCount > 0 && selectedCount < 2)
            {
                ...
                if (Convert.ToInt32(zt)==2)
                {
                    sqlstr = "update XXGLPT_XM set ZT=4 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        databind();
                        Alert.Show("操作成功");
        ...
        protected void Buttont_tj_Click(object sender, EventArgs e)
        {
            ...
            if (selectedCount > 0 && selectedCount < 2)
            {
                ...
                if (Convert.ToInt32(zt)==2)
                {
                    sqlstr = "update XXGLPT_XM set ZT=3 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        databind();
                        Alert.Show("操作成功");
