...
{
    public partial class JSJNJS_XMGL : System.Web.UI.Page
    {
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "up")
            {
                if (zt == "1" || zt == "4" || zt == "6")
                {
                    PageContext.RegisterStartupScript(Window1.GetShowReference("JSJNJS_XMGL_Edit.aspx?id=" + selectedID, "修改申报书") + Window1.GetMaximizeReference());
                }
                else
                {
            ...
            else if (e.CommandName == "tj")
            {
                if (zt == "1" || zt == "4" || zt == "6")
                {
                    string sqlstr = "update JSJNJS_XM set ZT=2 where ID=" + Convert.ToInt32(selectedID);
            ...
            else if (e.CommandName == "del")
            {
                if (zt == "1")
                {
                    string sqlstr = "update JSJNJS_XM set SFSC=1 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        databind();
                        Alert.Show("操作成功");
