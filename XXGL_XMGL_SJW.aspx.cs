...
{
    public partial class XXGL_XMGL_SJW : System.Web.UI.Page
    {
        ...
        protected void databind()
        {
            ...
            //sqlstr = "select * from YLZY  where SFSC!=1  and ZT>=3 and ZT!=4 order by XMBH";
            {
                ...
                //sqlstr = "SELECT '通用类' as XMDL, dbo.TYL_XM.ID, dbo.TYL_XM.XMBH, dbo.TYL_XM.XMMC, dbo.TYL_XM.JFYS_ZXJF, dbo.TYL_XM.JFYS_XXPTJF, dbo.XMFJ.XMYSMXWJM, dbo.TYL_XM.SFSC, dbo.TYL_XM.ZT, dbo.TYL_XM.User_Uid FROM dbo.TYL_XM LEFT JOIN dbo.XMFJ ON dbo.TYL_XM.XMBH = dbo.XMFJ.XMBH WHERE  SFSC!=1  and ZT>=3 and ZT!=4 and ZT!=6   order by dbo.TYL_XM.XMBH";
                sqlstr = "SELECT '信息管理平台' as XMDL, dbo.XXGLPT_XM.ID, dbo.XXGLPT_XM.XMBH, dbo.XXGLPT_XM.XMMC, dbo.XXGLPT_XM.JFYS_ZXJF, dbo.XXGLPT_XM.JFYS_XXPTJF, dbo.XMFJ.XMYSMXWJM, dbo.XXGLPT_XM.SFSC, dbo.XXGLPT_XM.ZT, dbo.XXGLPT_XM.User_Uid FROM dbo.XXGLPT_XM INNER JOIN dbo.XMFJ ON dbo.XXGLPT_XM.XMBH = dbo.XMFJ.XMBH WHERE  SFSC!=1  and ZT>=3 and ZT!=4 and ZT!=6   order by dbo.XXGLPT_XM.XMBH";
            ...
            //sqlstr = "select * from YLZY  where  SFSC!=1 and ZT>=3 and ZT!=4 and XXDM='" + ViewState["xxdm"].ToString() + "' order by XMBH";
            {
                ...
                sqlstr = "SELECT '信息管理平台' as XMDL, dbo.XXGLPT_XM.ID, dbo.XXGLPT_XM.XMBH, dbo.XXGLPT_XM.XMMC, dbo.XXGLPT_XM.JFYS_ZXJF, dbo.XXGLPT_XM.JFYS_XXPTJF, dbo.XMFJ.XMYSMXWJM, dbo.XXGLPT_XM.SFSC, dbo.XXGLPT_XM.ZT, dbo.XXGLPT_XM.User_Uid FROM dbo.XXGLPT_XM INNER JOIN dbo.XMFJ ON dbo.XXGLPT_XM.XMBH = dbo.XMFJ.XMBH WHERE  SFSC!=1  and ZT>=3 and ZT!=4 and ZT!=6 and DWMC='" + ViewState["xxmc"].ToString() + "'   order by dbo.XXGLPT_XM.XMBH";
                    DataTable dt = DbHelperSQL.Query(sqlstr).Tables[0];
                    Grid1.DataSource = dt;
                    Grid1.DataBind();
