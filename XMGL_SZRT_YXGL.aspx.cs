...
{
    public partial class XMGL_SZRT_YXGL : System.Web.UI.Page
    {
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "th")
            {
                ...
                {
                    string sqlstr = "update SZRT set ZT=4 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        databind();
                        Alert.Show("操作成功");
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            if (selectedCount > 0 && selectedCount < 2)
            {
                ...
                if (Convert.ToInt32(zt)==2)
                {
                    sqlstr = "update SZRT set ZT=4 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        databind();
                        Alert.Show("操作成功");
        ...
        protected void Buttont_tj_Click(object sender, EventArgs e)
        {
            ...
            if (selectedCount > 0 && selectedCount < 2)
            {
                ...
                if (Convert.ToInt32(zt)==2)
                {
                    sqlstr = "update SZRT set ZT=3 where ID=" + Convert.ToInt32(selectedID);
                    if (DbHelperSQL.ExecuteSql(sqlstr) != 0)
                    {
                        databind();
                        Alert.Show("操作成功");
