...
{
    public partial class XMGL_SJW : System.Web.UI.Page
    {
        ...
        protected void databind_DropDownList1()
        {
            ...
            //DataTable dt = DbHelperSQL.Query("select XMSBSMB.ID AS ID, XMSBSMB.XMSBSMBMC as XMSBSMBMC from XMSBSMB_YX,XMSBSMB where XMSBSMB_YX.ID=XMSBSMB.ID and XMSBSMB_YX.XXDM='" + xxdm + "'").Tables[0];
        ...
        protected void databind()
        {
            ...
            //sqlstr = "select * from YLZY  where SFSC!=1  and ZT>=3 and ZT!=4 order by XMBH";
            {
                if (ViewState["xmld"].ToString() == "所有")
                {
                    ...
                    sqlstr = "select c.XMBH,c.ID,c.XMDL,c.XMKXXFXBGWJM,c.XXDM,c.YXXSALWJM,c.XMYSMXWJM,c.ZT,c.ZYDM,c.ZYMC from (select  a.ID,a.XXDM,  a.XMBH,'养老服务类示范专业' as XMDL,a.ZT,a.ZYDM,a.ZYMC, XMFJ.XMKXXFXBGWJM,XMFJ.YXXSALWJM,XMFJ.XMYSMXWJM from ( select ID,XXDM,XMBH,ZT,ZYDM,ZYMC from YLFW where   SFSC!=1  and ZT>=3 and ZT!=4 and ZT!=6) as a left join XMFJ  on   a.XMBH=XMFJ.XMBH ) as c,XXJBQKB as b where c.XXDM=b.XXDM  order by c.XMBH";
            ...
            //sqlstr = "select * from YLZY  where  SFSC!=1 and ZT>=3 and ZT!=4 and XXDM='" + ViewState["xxdm"].ToString() + "' order by XMBH";
            {
                if (ViewState["xmld"].ToString() == "所有")
                {
                    ...
                    sqlstr = "select c.XMBH,c.ID,c.XMDL,c.XMKXXFXBGWJM,c.XXDM,c.YXXSALWJM,c.XMYSMXWJM,c.ZT,c.ZYDM,c.ZYMC from (select  a.ID,a.XXDM,  a.XMBH,'养老服务类示范专业' as XMDL,a.ZT,a.ZYDM,a.ZYMC, XMFJ.XMKXXFXBGWJM,XMFJ.YXXSALWJM,XMFJ.XMYSMXWJM from ( select ID,XXDM,XMBH,ZT,ZYDM,ZYMC from YLFW where   SFSC!=1  and ZT>=3 and ZT!=4 and ZT!=6 and XXDM='" + ViewState["xxdm"].ToString() + "') as a left join XMFJ  on   a.XMBH=XMFJ.XMBH ) as c,XXJBQKB as b where c.XXDM=b.XXDM  order by c.XMBH";
                    DataTable dt = DbHelperSQL.Query(sqlstr).Tables[0];
                    Grid2_ylfw.DataSource = dt;
                    Grid2_ylfw.DataBind();
